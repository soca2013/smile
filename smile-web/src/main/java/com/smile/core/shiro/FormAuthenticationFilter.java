...
public class FormAuthenticationFilter extends AuthenticatingFilter {
    ...
    protected void setFailureAttribute(ServletRequest request, AuthenticationException ae) {
        if (ae instanceof UnknownAccountException) {
            request.setAttribute(getFailureKeyAttribute(), "账号不存在");
        } else if (ae instanceof UnknownAccountException) {
            request.setAttribute(getFailureKeyAttribute(), "账号不存在");
        } else if (ae instanceof UnsupportedTokenException) {
            request.setAttribute(getFailureKeyAttribute(), "无效token");
            ...
            request.setAttribute(getFailureKeyAttribute(), "账号密码不正确");
        }
